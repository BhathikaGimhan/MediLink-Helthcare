...
const Doctors: React.FC = () => {
      ...
      (err) => {
        ...
        if (err.code === 'failed-precondition' && err.message.includes('index')) {
          ...
          if (match) {
            ...
            setError('This query requires a Firestore index. Please create it in the Firebase Console.');
          }
        } else {
    ...
    posts.forEach((post) => {
      ...
      const unsubscribe = onSnapshot(
        ...
        (err) => {
          ...
          setError('Failed to fetch comments for post ' + post.id + ': ' + err.message);
        }
      );
  ...
  const handlePostSubmit = async () => {
    ...
    if (user.role !== 'admin-1') {
      setError('Only admin-1 doctors can post.');
      return;
    }
    if (!newPost.trim()) {
  ...
  return (
    ...
    >
      <div className="container mx-auto max-w-5xl">
        <h2 className="text-3xl font-bold neon-text text-cyan-400 mb-6">Doctors' Community</h2>
        {error && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        {/* Post box visible only for admin-1 */}
        {!isLoading && user?.role === 'admin-1' && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        {!isLoading && postsLoading && <p className="text-gray-400 text-center">Loading posts...</p>}
          ...
          <div className="space-y-6">
            {posts.length > 0 ? (
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-6 mt-4">
                        <button
                          ...
                          onClick={() =>
                            handleLikeToggle(post.id, post.likes.includes(user?.id || ''))
                          }
                        >
                          <Heart
                            className={`w-4 h-4 ${
                            ...
                            fill={
                              post.likes.includes(user?.id || '') ? 'currentColor' : 'none'
                            }
                          />
              ...
              <p className="text-gray-400 text-center">
                No posts yet in the Doctors' Community.
              </p>
            )}
          </div>
        )}
